...
export function RastreamentoModal({ entrega, lojaId, token, onClose }: Props) {
  const { entregadorLocation, connected } = useRastreamento({ entregaId: entrega.id, lojaId, token });
          ...
          {!entregadorLocation && (
            <View style={s.mapPlaceholder}>
              ...
              <Text style={s.mapPlaceholderTxt}>
                {connected ? 'Aguardando localização do entregador...' : 'Conectando ao rastreamento...'}
              </Text>
        ...
        <ScrollView style={s.infoScroll} contentContainerStyle={{ padding: 16, gap: 10 }}>
          ...
          <View style={s.infoCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.infoCardSub} numberOfLines={1}>{entrega.endereco}</Text>
            </View>
          </View>
        </ScrollView>
      </SafeAreaView>
