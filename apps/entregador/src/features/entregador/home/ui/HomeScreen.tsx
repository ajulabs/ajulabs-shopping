...
export function HomeScreen({
  ...
  return (
    <SafeAreaView style={s.safeArea}>
      ...
      <View style={s.topBar}>
        <TouchableOpacity
          style={[s.onlineToggle, { backgroundColor: online ? '#39FF89' : theme.surf }]}
        ...
        >
          <View
            style={[
              s.onlineIcon,
              { backgroundColor: online ? 'rgba(255,255,255,0.3)' : theme.surf2 },
            ]}
          >
            <Ionicons name="power" size={18} color={online ? '#002B12' : theme.textMut} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={[s.onlineTitle, { color: online ? '#002B12' : theme.text }]}>
              {online ? 'Você está online' : 'Você está offline'}
            </Text>
            <Text style={[s.onlineSub, { color: online ? '#046C2E' : theme.textMut }]}>
          ...
          <View
            style={[
              s.toggleTrack,
              { backgroundColor: online ? 'rgba(0,43,18,0.25)' : theme.border },
            ]}
          >
...
function makeStyles(theme: Theme) {
  return StyleSheet.create({
    ...
    summaryAmount: { fontSize: 28, fontWeight: '800', color: theme.text },
    summarySub: { fontSize: 11, color: theme.textMut, marginTop: 3 },
    dividerV: { width: 1, height: 36, backgroundColor: theme.border },
    summaryCol: { flex: 1, alignItems: 'center' },
    summaryColVal: { fontSize: 20, fontWeight: '700', color: theme.text },
    summaryColLabel: { fontSize: 10, color: theme.textMut, marginTop: 4 },
    ...
    waitingCard: { backgroundColor: theme.surf2, borderRadius: 12, padding: 12, marginBottom: 10 },
    waitingCardBody: { flexDirection: 'row', gap: 10 },
    waitingRoute: { flex: 1 },
    waitingRouteRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
    waitingRouteMain: { fontSize: 13, fontWeight: '600', color: theme.text },
    waitingRouteSub: { fontSize: 11, color: theme.textMut, marginTop: 1 },
    waitingCardRight: { alignItems: 'flex-end', justifyContent: 'space-between', gap: 4 },
    waitingGanho: { fontSize: 16, fontWeight: '800', color: theme.text },
    waitingDuracao: { fontSize: 10, color: theme.textMut },
    btnWaitingAccept: {
      paddingHorizontal: 14,
      paddingVertical: 8,
      borderRadius: 10,
