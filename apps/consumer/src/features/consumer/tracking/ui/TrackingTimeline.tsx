...
const STEPS = [
  ...
  {
    key: 'preparando',
    label: 'Loja preparando',
    desc: 'A loja confirmou e está separando os itens',
  },
  { key: 'pronto', label: 'Pedido pronto', desc: 'Pronto! Aguardando um entregador retirar' },
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <View key={step.key} style={styles.row}>
            {!isLast && (
              <View
                style={[styles.line, { backgroundColor: done ? colors.orange : pendingLine }]}
              />
            )}
            ...
            <View style={styles.textBox}>
              ...
              {active && <Text style={[styles.desc, { color: descColor }]}>{step.desc}</Text>}
            </View>
          </View>
        );
      })}
