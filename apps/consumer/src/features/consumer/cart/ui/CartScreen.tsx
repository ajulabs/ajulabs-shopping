...
import {
  ...
  Modal, TextInput, ActivityIndicator, Alert, Platform, KeyboardAvoidingView,
} from 'react-native';
...
interface EnderecoForm {
  apelido: string; rua: string; numero: string;
  bairro: string; cep: string; cidade: string; complemento: string;
}
const FORM_VAZIO: EnderecoForm = {
  apelido: '', rua: '', numero: '', bairro: '', cep: '', cidade: 'Aracaju', complemento: '',
};
...
export function CartScreen() {
  ...
  const buscarCep = useCallback(async (digits: string) => {
    ...
    setBuscandoCep(true); clearError('cep');
    try {
      const res  = await fetch(`https://viacep.com.br/ws/${digits}/json/`);
      const data = await res.json();
      if (data.erro) { setErrors(e => ({ ...e, cep: 'CEP não encontrado.' })); return; }
      setForm(f => ({ ...f, rua: data.logradouro || f.rua, bairro: data.bairro || f.bairro, cidade: data.localidade || f.cidade }));
    } catch { setErrors(e => ({ ...e, cep: 'Erro ao buscar CEP.' })); }
    finally  { setBuscandoCep(false); }
  }, []);

  const usarLocalizacao = async () => {
    setBuscandoLoc(true); setErroLoc('');
    try {
  ...
  const fecharForm = () => {
    setShowForm(false);
    setForm(FORM_VAZIO);
    setErrors({}); setErroGeral(''); setErroLoc('');
  };
  ...
  const handleSalvar = async () => {
    ...
    try {
      const novo = await EnderecoService.criar(token, {
        ...form, cep: form.cep.replace(/\D/g, ''), complemento: form.complemento || undefined,
      });
      fecharForm();
      setShowPicker(true);
    ...
    return (
      <View style={[styles.container, { backgroundColor: bg }]}>
        <View style={[styles.header, { backgroundColor: surf, borderBottomColor: borderL }]}>
          <TouchableOpacity onPress={() => router.push('/(consumer)/vitrines')} style={[styles.btnBack, { backgroundColor: backBtn }]}>
        ...
        <View style={styles.vazio}>
          ...
          <Text style={[styles.vazioTxt, { color: textSec as string }]}>Explore as vitrines e adicione produtos</Text>
          <TouchableOpacity style={styles.vazioBtn} onPress={() => router.push('/(consumer)/vitrines')} activeOpacity={0.85}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: bg }]}>
      {/* Header */}
      <View style={[styles.header, { backgroundColor: surf, borderBottomColor: borderL }]}>
        <TouchableOpacity onPress={() => router.push('/(consumer)/vitrines')} style={[styles.btnBack, { backgroundColor: backBtn }]} activeOpacity={0.85}>
          <Ionicons name="chevron-back" size={20} color={text} />
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
          <Text style={[styles.titulo, { color: text }]}>Carrinho</Text>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.endCard, { backgroundColor: surf, borderColor: border }]}>
          <TouchableOpacity style={styles.endRow} onPress={() => setShowPicker(v => !v)} activeOpacity={0.8}>
            ...
            <View style={{ flex: 1 }}>
              {enderecoAtual ? (
                <>
                  ...
                  <Text style={[styles.endRua, { color: textSec as string }]} numberOfLines={1}>{enderecoAtual.rua}</Text>
                </>
              ) : (
            ...
            <Ionicons name={showPicker ? 'chevron-up' : 'chevron-down'} size={16} color={textSec as string} />
          </TouchableOpacity>
            ...
            <View style={[styles.endLista, { borderTopColor: borderL }]}>
              {enderecos.map(addr => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.75}
                >
                  <Ionicons name={addr.id === enderecoId ? 'radio-button-on' : 'radio-button-off'} size={18} color={colors.orange} />
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.endOpcaoApelido, { color: text }]}>
                      {addr.apelido}{addr.padrao ? '  ·  Padrão' : ''}
                    </Text>
                    <Text style={[styles.endOpcaoRua, { color: textSec as string }]} numberOfLines={1}>{addr.rua}</Text>
                  </View>
                </TouchableOpacity>
              ))}
              ...
              <TouchableOpacity
                style={[styles.endOpcaoAdd, { borderTopColor: borderL, borderTopWidth: enderecos.length > 0 ? 1 : 0 }]}
                onPress={() => { setShowPicker(false); setShowForm(true); }}
                activeOpacity={0.75}
              >
        ...
        {numLojas > 1 && (
          <View style={[styles.banner, isDark && { backgroundColor: 'rgba(242,118,15,0.12)' }]}>
            ...
            <Text style={styles.bannerTxt}>Cada loja tem seu frete e motoboy próprio, mas você paga tudo de uma vez.</Text>
          </View>
        )}

        {grupos.map((grupo, idx) => (
          <CartLojaGrupo key={grupo.lojaId} numero={idx + 1} grupo={grupo} onAumentar={aumentar} onDiminuir={diminuir} />
        ))}
        ...
        <View style={[styles.totalCard, { backgroundColor: surf, borderColor: border }]}>
          ...
          <View style={styles.linhaTotal}>
            <Text style={[styles.linhaLabel, { color: textSec as string }]}>Frete{numLojas > 1 ? ` (${numLojas} lojas)` : ''}</Text>
            <Text style={[styles.linhaValue, { color: text }]}>{freteTotal === 0 ? 'Grátis' : fmt(freteTotal)}</Text>
          </View>
      ...
      <View style={[styles.footer, { backgroundColor: surf, borderTopColor: borderL }]}>
        <TouchableOpacity style={styles.btnFinalizar} onPress={() => router.push('/(consumer)/checkout')} activeOpacity={0.9}>
      ...
      <Modal visible={showForm} animationType="slide" presentationStyle="pageSheet">
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={[styles.modal, { backgroundColor: bg }]}>
            ...
            <View style={[styles.modalHeader, { backgroundColor: surf, borderBottomColor: borderL }]}>
            ...
            <ScrollView contentContainerStyle={styles.modalScroll} showsVerticalScrollIndicator={false}>
              ...
              {/* Usar localização */}
              <TouchableOpacity style={[styles.locBtn, buscandoLoc && { opacity: 0.6 }]} onPress={usarLocalizacao} disabled={buscandoLoc} activeOpacity={0.8}>
                {buscandoLoc
                  ? <ActivityIndicator size="small" color={colors.orange} />
                  : <Ionicons name="navigate-outline" size={16} color={colors.orange} />
                }
                <Text style={styles.locBtnTxt}>{buscandoLoc ? 'Buscando localização...' : 'Usar minha localização'}</Text>
              </TouchableOpacity>
              {!!erroLoc && (
              ...
              <View style={styles.cepRow}>
                <TextInput
                  ...
                  onChangeText={v => {
                    const d = v.replace(/\D/g, '').slice(0, 8);
                    setForm(f => ({ ...f, cep: d })); clearError('cep');
                  ...
                  maxLength={9}
                />
                {buscandoCep && <ActivityIndicator size="small" color={colors.orange} style={{ marginLeft: 10 }} />}
              </View>
              ...
              <TouchableOpacity style={[styles.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSalvar} disabled={saving} activeOpacity={0.85}>
                {saving ? <ActivityIndicator color={colors.n0} /> : <Text style={styles.saveBtnTxt}>Salvar endereço</Text>}
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  linhaTotal:      { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 4 },
  ...
  endOpcaoAdd:     { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 12, marginTop: 2 },
  ...
  vazioBtn:        { marginTop: 16, paddingHorizontal: 24, paddingVertical: 12, backgroundColor: colors.orange, borderRadius: 12 },
