...
import { Produto, AvaliacaoLoja } from '@ajulabs/types';
...
// Tamanhos típicos por categoria — usado quando não há variações cadastradas
...
function categoriaTamanho(categoria: string): string[] | null {
  ...
  if (c.includes('roupa') || c.includes('camisa') || c.includes('camiseta') ||
      c.includes('calça') || c.includes('blusa') || c.includes('moletom')) {
    return TAMANHOS_POR_CATEGORIA.roupa;
  }
  if (c.includes('calçado') || c.includes('tênis') || c.includes('sapato') ||
      c.includes('chinelo') || c.includes('sandália')) {
...
function TabelaTamanho({ opcoes, isDark, text, textSec, borderL }: {
  opcoes: string[]; isDark: boolean;
  text: string; textSec: string; borderL: string;
}) {
  const [selecionado, setSelecionado] = useState<string | null>(null);

  return (
    <View>
      <Text style={[styles.tamanhoLabel, { color: textSec }]}>Tamanho do produto:</Text>
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                styles.tamanhoBadge,
                {
                  backgroundColor: ativo ? colors.navy : (isDark ? 'rgba(255,255,255,0.06)' : colors.n0),
...
function AvaliacaoResumo({ avaliacoes, text, textSec, borderL }: {
  ...
  const media = Math.round(
    (avaliacoes.reduce((s, a) => s + a.nota, 0) / avaliacoes.length) * 10,
  ) / 10;
    ...
    <View style={[styles.avResumoCard, { borderColor: borderL }]}>
      ...
      <View style={styles.avResumoTop}>
        <View style={styles.avResumoEsq}>
          ...
          <Text style={[styles.avRecomenda, { color: textSec }]}>
...
function AvaliacaoItem({ av, text, textSec, surf, borderL }: {
  av: AvaliacaoLoja;
  text: string; textSec: string; surf: string; borderL: string;
}) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.simInfo}>
        <Text style={[styles.simNome, { color: text }]} numberOfLines={2}>{produto.nome}</Text>
...
export function ProdutoDetail({ produtoId }: ProdutoDetailProps) {
  ...
  const handleAdd = useCallback((p: Produto = produto!) => {
    if (!p) return;
    adicionar(p);
    if (p.id === produtoId) {
      setAdded(true);
      Animated.sequence([
        ...
        Animated.spring(addScale, { toValue: 1, useNativeDriver: true, speed: 20, bounciness: 6 }),
      ]).start();
    }
  }, [produto, adicionar, produtoId]);
  ...
  // Tamanhos: usa variações cadastradas ou inferidas da categoria
  const tamanhosVariacao = variacoes.map(v => v.nome);
  const tamanhosInferidos = tamanhosVariacao.length === 0
    ? (categoriaTamanho(produto.categoria) ?? [])
    : [];
  const tamanhosMostrar = tamanhosVariacao.length > 0 ? tamanhosVariacao : tamanhosInferidos;
  ...
  return (
    <View style={[styles.container, { backgroundColor: bg }]}>
      ...
      {/* Header */}
      <View style={[styles.header, { backgroundColor: surf, borderBottomColor: borderL as string }]}>
        <TouchableOpacity onPress={() => router.push(`/(consumer)/vitrine/${produto.lojaId}` as any)} style={[styles.headerBtn, { backgroundColor: backBtn }]}>
          <Ionicons name="chevron-back" size={20} color={text} />
        </TouchableOpacity>
        <Text style={[styles.headerTitulo, { color: text }]} numberOfLines={1}>{produto.nome}</Text>
        <Animated.View style={{ transform: [{ scale: heartScale }] }}>
          <TouchableOpacity onPress={handleFavorito} style={[styles.headerBtn, { backgroundColor: backBtn }]}>
            <Ionicons
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={[styles.infoCard, { backgroundColor: surf, borderColor: borderL as string }]}>
          ...
          {/* Tabela de tamanho inline */}
          {tamanhosMostrar.length > 0 && (
            <View style={[styles.tamanhoSection, { borderTopColor: borderL as string }]}>
              <TabelaTamanho
                opcoes={tamanhosMostrar}
                isDark={isDark}
                text={text}
        ...
        <View style={[styles.section, { backgroundColor: surf, borderColor: borderL as string }]}>
          ...
          <InfoRow label="Categoria" value={produto.categoria}
            text={text} textSec={textSec as string} borderL={borderL as string} />
...
function InfoRow({ label, value, text, textSec, borderL }: {
...
const styles = StyleSheet.create({
  ...
  header:               { flexDirection: 'row', alignItems: 'center', gap: 10,
                          ...
                          borderBottomWidth: 1 },
  headerBtn:            { width: 38, height: 38, borderRadius: 19,
  ...
  // Info card
  infoCard:             { margin: 16, marginTop: 12, borderRadius: 16,
  ...
  tamanhoBadge:         { minWidth: 44, height: 36, borderRadius: 8, borderWidth: 1.5,
                          alignItems: 'center', justifyContent: 'center',
  ...
  infoRow:              { flexDirection: 'row', justifyContent: 'space-between',
                          alignItems: 'flex-start', paddingVertical: 10, borderTopWidth: 1 },
  infoLabel:            { fontSize: 13, fontWeight: '500', flex: 1 },
  infoValue:            { fontSize: 13, fontWeight: '600', flex: 2, textAlign: 'right' },
  tagsWrap:             { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  tagChip:              { paddingHorizontal: 10, paddingVertical: 5,
                          borderRadius: 99, borderWidth: 1 },
  tagTxt:               { fontSize: 12 },

  // Avaliações — resumo
  avResumoCard:         { borderRadius: 12, borderWidth: 1,
  ...
  btnVerTodas:          { marginTop: 14, paddingVertical: 11, borderRadius: 10,
  ...
  simImgFallback:       { width: '100%', aspectRatio: 4 / 3,
                          backgroundColor: colors.orange100,
                          alignItems: 'center', justifyContent: 'center' },
  simImgFallbackTxt:    { fontSize: 28, fontWeight: '700', color: colors.orange600 },
  simBadge:             { position: 'absolute', top: 6, right: 6,
  ...
  simBtnAdd:            { marginTop: 8, paddingVertical: 5, borderRadius: 8,
                          backgroundColor: colors.orange100,
                          alignItems: 'center' },
  ...
  btnAdd:               { height: 52, borderRadius: 14, flexDirection: 'row',
  ...
  btnVoltar:            { marginTop: 16, paddingHorizontal: 24, paddingVertical: 12,
