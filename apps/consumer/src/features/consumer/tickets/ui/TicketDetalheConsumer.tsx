            ...
            <View style={tl.iconCol}>
              <View style={[tl.dot, done && tl.dotDone, current && tl.dotCurrent]}>
                <Ionicons
                  name={step.icon}
                  size={14}
                  color={done ? colors.n0 : colors.n300}
                />
              </View>
              {i < STEPS.length - 1 && (
...
const tl = StyleSheet.create({
  ...
  dot:         { width: 28, height: 28, borderRadius: 14, backgroundColor: colors.n200,
...
function AvaliacaoStars({ nota, onAvaliar }: { nota: number | null; onAvaliar: (n: number) => void }) {
  ...
  return (
    <View style={av.wrap}>
      ...
      <View style={av.stars}>
        {[1,2,3,4,5].map(n => (
          <TouchableOpacity key={n} onPress={() => onAvaliar(n)} activeOpacity={0.8}>
            <Ionicons name={n <= hover ? 'star' : 'star-outline'} size={28} color="#F59E0B"
              onLayout={() => {}} />
          </TouchableOpacity>
        ))}
      </View>
      <View style={av.stars}>
        {[1,2,3,4,5].map(n => (
          <TouchableOpacity key={n} onPress={() => { setHover(n); onAvaliar(n); }} activeOpacity={0.8}
            style={{ padding: 4 }}>
...
export function TicketDetalheConsumer() {
    ...
    try {
      ...
      setTicket(t => t ? {
        ...
        mensagens: [...t.mensagens, {
          id: nova.id, remetente: 'consumidor',
          texto: nova.texto, criadoEm: nova.criadoEm ?? nova.criado_em,
        }],
      } : t);
      setMsg('');
  ...
  async function cancelarTicket() {
    ...
    Alert.alert(
      'Cancelar ticket',
      'Tem certeza que deseja cancelar esta reclamação?',
      [
        { text: 'Não', style: 'cancel' },
        {
          text: 'Cancelar ticket', style: 'destructive',
          onPress: async () => {
            setCancelando(true);
            try {
  ...
  return (
    ...
    >
      ...
      <View style={[s.header, { backgroundColor: surf, borderBottomColor: borderL }]}>
        <TouchableOpacity onPress={() => router.push('/(consumer)/tickets' as any)} style={s.backBtn} activeOpacity={0.7}>
          <Ionicons name="arrow-back" size={20} color={text} />
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
          <Text style={[s.protocolo, { color: text }]}>{ticket.protocolo}</Text>
          <Text style={[s.lojaHeader, { color: textSec as string }]}>{ticket.loja?.nome ?? '—'}</Text>
        </View>
        ...
        {ticket.pedido && (
          <View style={[s.section, { backgroundColor: surf, borderColor: borderL }]}>
            ...
            <View style={s.pedidoRow}>
              <Text style={[s.pedidoId, { color: text }]}>#{ticket.pedido.id.slice(-8).toUpperCase()}</Text>
              <Text style={[s.pedidoTotal, { color: colors.orange }]}>{brl(ticket.pedido.total)}</Text>
            </View>
        ...
        <View style={[s.section, { backgroundColor: surf, borderColor: borderL }]}>
          ...
          {ticket.mensagens.length === 0 ? (
            ...
            ticket.mensagens.map((m: TicketMensagem) => (
              ...
              >
                <Text style={[s.msgRem, { color: m.remetente === 'consumidor' ? colors.orange600 : colors.navy }]}>
                  {m.remetente === 'consumidor' ? 'Você' : 'Loja'}
                </Text>
                <Text style={[s.msgTxt, { color: text }]}>{m.texto}</Text>
                <Text style={[s.msgData, { color: textMut as string }]}>{dataHora(m.criadoEm)}</Text>
              </View>
            ))
          )}
        </View>
...
const s = StyleSheet.create({
  ...
  header:       { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16,
                  paddingTop: 52, paddingBottom: 14, borderBottomWidth: 1, gap: 10 },
  backBtn:      { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.n100,
                  alignItems: 'center', justifyContent: 'center' },
  ...
  sectionTitle: { fontSize: 10.5, fontWeight: '700', textTransform: 'uppercase',
  ...
  cancelarBtn:  { alignItems: 'center', padding: 14, borderRadius: 12, borderWidth: 1.5,
                  borderColor: '#DC2626', marginBottom: 4 },
  cancelarBtnTxt: { fontSize: 14, fontWeight: '600', color: '#DC2626' },
  inputWrap:    { flexDirection: 'row', alignItems: 'flex-end', gap: 8, padding: 12,
                  borderTopWidth: 1 },
  input:        { flex: 1, borderWidth: 1, borderRadius: 12, paddingHorizontal: 12,
                  paddingVertical: 10, fontSize: 13, maxHeight: 100 },
  sendBtn:      { width: 40, height: 40, borderRadius: 20, backgroundColor: colors.navy,
                  alignItems: 'center', justifyContent: 'center' },
});
