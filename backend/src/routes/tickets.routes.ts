...
const TICKET_INCLUDE = {
  ...
  pedido:    { select: { id: true, total: true, criadoEm: true, itens: { select: { nomeSnapshot: true, quantidade: true } } } },
  mensagens: true,
} as const;
...
router.post('/:id/mensagens', authMiddleware, authUsuario, async (req: AuthRequest, res) => {
  try {
    ...
    if (ticket.status === 'cancelado' || ticket.status === 'resolvido') {
      return res.status(400).json({ error: 'Não é possível enviar mensagem em ticket finalizado' });
    }
...
router.post('/:id/avaliacao', authMiddleware, authUsuario, async (req: AuthRequest, res) => {
  try {
    ...
    if (error instanceof z.ZodError) return res.status(400).json({ error: 'Nota deve ser de 1 a 5' });
