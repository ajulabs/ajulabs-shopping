...
function entregadorMarkerHtml(heading: number): string {
  return `
    <div style="
    ...
    ">
      <div style="
        ...
        background:rgba(32,156,239,0.18);
      "></div>
      <div style="
...
export function DeliveryTrackingMap({
  ...
  useEffect(() => {
    loadLeaflet(() => {
      ...
      tileLayerRef.current = L.tileLayer(isDark ? TILE_DARK : TILE_LIGHT, { maxZoom: 19 }).addTo(map);
  ...
  useEffect(() => {
    ...
    tileLayerRef.current = L.tileLayer(isDark ? TILE_DARK : TILE_LIGHT, { maxZoom: 19 }).addTo(mapRef.current);
  ...
  useEffect(() => {
    ...
    if (entregadorMarkerRef.current) {
      try {
        ...
        entregadorMarkerRef.current.setIcon(L.divIcon({
          className: '',
          html: entregadorMarkerHtml(entregadorLocation.heading ?? 0),
          iconSize: [46, 46],
          iconAnchor: [23, 23],
        }));
      } catch {
  ...
  }, [isReady, entregadorLocation?.lat, entregadorLocation?.lng, entregadorLocation?.heading, following]);
  ...
  useEffect(() => {
    ...
    if (useProvided) {
      ...
      else routePolyRef.current = L.polyline(latLngs, { color: '#209CEF', weight: 5, opacity: 0.9, lineCap: 'round' }).addTo(mapRef.current);
    ...
    fetchRouteWeb(entregadorLocation, destination).then(latLngs => {
      ...
      if (routePolyRef.current) routePolyRef.current.setLatLngs(latLngs);
      else routePolyRef.current = L2.polyline(latLngs, { color: '#209CEF', weight: 5, opacity: 0.9, lineCap: 'round' }).addTo(mapRef.current);
      routePolyRef.current.bringToBack();
    });
  }, [
    isReady,
  ...
  return (
     ...
     */
    <div style={{ position: 'relative', flex: 1, overflow: 'hidden', zIndex: 0, ...(style as any) }}>
      ...
      {!isFinished && etaSeconds != null && etaSeconds > 0 && (
        <div style={{
          ...
          <span style={{ fontSize: 18, fontWeight: 800, color: textPrimary }}>{fmtEta(etaSeconds)}</span>
          {distanceRemaining != null && distanceRemaining > 0 && (
            <>
              <div style={{ width: 1, height: 18, background: isDark ? '#2E3555' : '#E4E7F1' }} />
              <span style={{ fontSize: 14, fontWeight: 600, color: textMuted }}>{fmtDist(distanceRemaining)}</span>
            </>
          )}
        </div>
      )}
      ...
      {isNavigationMode && currentStep && (
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', lineHeight: '19px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        <div style={{
          ...
          {[
            ...
            <React.Fragment key={stat.label}>
              {i > 0 && <div style={{ width: 1, height: 28, background: isDark ? '#2E3555' : '#E4E7F1' }} />}
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontSize: 9, fontWeight: 700, color: textMuted, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{stat.label}</div>
      ...
      {isOffRoute && (
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>↺  Recalculando rota...</span>
        </div>
      )}
      ...
      {statusCfg && (
        <div style={{
          position: 'absolute', bottom: isNavigationMode ? 90 : 24,
          ...
          <span style={{ fontSize: 12.5, fontWeight: 700, color: statusCfg.color }}>{statusCfg.label}</span>
        </div>
      )}
      ...
      {!following && (
        <button
          ...
          style={{
            position: 'absolute', bottom: isNavigationMode ? 100 : 70, right: 16,
            width: 46, height: 46, borderRadius: '50%',
            background: bg, border: 'none', cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: '0 5px 16px rgba(0,0,0,.25)',
            fontSize: 22, color: isDark ? '#fff' : '#209CEF',
            zIndex: 500,
          }}
        >⊕</button>
      )}
      ...
      {!isNavigationMode && entregadorLocation && destination && (
        <button
          onClick={fitRoute}
          style={{
            position: 'absolute', bottom: 126, right: 16,
            width: 42, height: 42, borderRadius: '50%',
            background: bg, border: 'none', cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: '0 4px 12px rgba(0,0,0,.2)',
            fontSize: 18, color: isDark ? '#fff' : '#000933',
            zIndex: 500,
          }}
        >⊞</button>
      )}
